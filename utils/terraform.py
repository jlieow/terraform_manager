...
def terraform_chaos_test():

    CHAOS_TEST_PREFACE = "The following directories are chaos test directories:\n"
    CHAOS_TEST_QUESTION = "\nWhich chaos test would you like to invoke: "
    ...
    # Get list of terraform roots
    list_chaos_test_dir = locate_chaos_test_directories(get_parent_dir(os.getcwd()))

    while True:
        # Question 1
        DIR_NUMBER = input_options(CHAOS_TEST_PREFACE, [os.path.basename(directory) for directory in list_chaos_test_dir], CHAOS_TEST_QUESTION,  allow_special_break=True, special_break="<")
        if DIR_NUMBER == "<":
            print()
            break
            
        print("\n\"%d. %s\" was selected." % (DIR_NUMBER+1, os.path.basename(list_chaos_test_dir[DIR_NUMBER])))

        COMMAND_NUMBER = None

        cwd = list_chaos_test_dir[DIR_NUMBER]

        # print("\ndoes_workflow_file_exist: %s"does_workflow_file_existist(cwd))
        
        chaos_test_file_exist = does_chaos_test_file_exist(cwd)
        if chaos_test_file_exist:
            print("chaos_test_file_exist")
